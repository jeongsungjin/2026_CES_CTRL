...
class Env_Manager():
    ...
    def tick(self, input_data):
        ...
        # 에고 차량 위치 정보
        ego_transform = self.manager.ego_vehicles[0].get_transform()
        ego_location = ego_transform.location
        ego_rotation = ego_transform.rotation
        
        # 가로등 카메라 데이터 처리
        streetlight_cam_1 = input_data.get('STREETLIGHT_CAM_1', [None, None])[1]
        streetlight_cam_2 = input_data.get('STREETLIGHT_CAM_2', [None, None])[1]
        
        if streetlight_cam_1 is not None:
            streetlight_cam_1 = streetlight_cam_1[:, :, :3]  # RGB 채널만 사용
        
        if streetlight_cam_2 is not None:
            streetlight_cam_2 = streetlight_cam_2[:, :, :3]  # RGB 채널만 사용
        
        # 가로등 카메라 깊이 데이터
        streetlight_cam_1_depth = input_data.get('STREETLIGHT_CAM_1_DEPTH', [None, None])[1]
        streetlight_cam_2_depth = input_data.get('STREETLIGHT_CAM_2_DEPTH', [None, None])[1]
        
        if streetlight_cam_1_depth is not None:
            streetlight_cam_1_depth = streetlight_cam_1_depth[:, :, :3]
            streetlight_cam_1_gray_depth = convert_depth(streetlight_cam_1_depth)
        else:
            streetlight_cam_1_gray_depth = None
            
        if streetlight_cam_2_depth is not None:
            streetlight_cam_2_depth = streetlight_cam_2_depth[:, :, :3]
            streetlight_cam_2_gray_depth = convert_depth(streetlight_cam_2_depth)
        else:
            streetlight_cam_2_gray_depth = None
            
        # 가로등 카메라 세그멘테이션 데이터
        streetlight_cam_1_sem_seg = input_data.get('STREETLIGHT_CAM_1_SEM_SEG', [None, None])[1]
        streetlight_cam_2_sem_seg = input_data.get('STREETLIGHT_CAM_2_SEM_SEG', [None, None])[1]
        
        if streetlight_cam_1_sem_seg is not None:
            streetlight_cam_1_sem_seg = streetlight_cam_1_sem_seg[:, :, 2]  # 세그멘테이션 클래스 채널
        
        if streetlight_cam_2_sem_seg is not None:
            streetlight_cam_2_sem_seg = streetlight_cam_2_sem_seg[:, :, 2]  # 세그멘테이션 클래스 채널
        
        # 가로등 카메라와 에고 차량 간의 변환 행렬 계산
        # 카메라 1 변환 행렬
        cam1_to_world = self.get_camera_to_world_matrix(
            self.sensors_mapping['STREETLIGHT_CAM_1']
        )
        
        # 카메라 2 변환 행렬
        cam2_to_world = self.get_camera_to_world_matrix(
            self.sensors_mapping['STREETLIGHT_CAM_2']
        )
        
        # 에고 차량에서 월드로의 변환 행렬
        ego_to_world = self.get_vehicle_to_world_matrix(ego_transform)
        
        # 카메라에서 에고 차량으로의 변환 행렬 (월드 좌표계를 통해)
        cam1_to_ego = np.linalg.inv(ego_to_world) @ cam1_to_world
        cam2_to_ego = np.linalg.inv(ego_to_world) @ cam2_to_world
        ...
        results = {
                ...
                # 가로등 카메라 데이터
                'streetlight_cam_1': streetlight_cam_1,
                'streetlight_cam_2': streetlight_cam_2,
                'streetlight_cam_1_depth': streetlight_cam_1_depth,
                'streetlight_cam_2_depth': streetlight_cam_2_depth,
                'streetlight_cam_1_gray_depth': streetlight_cam_1_gray_depth,
                'streetlight_cam_2_gray_depth': streetlight_cam_2_gray_depth,
                'streetlight_cam_1_sem_seg': streetlight_cam_1_sem_seg,
                'streetlight_cam_2_sem_seg': streetlight_cam_2_sem_seg,
                
                # 변환 행렬
                'cam1_to_ego': cam1_to_ego.tolist(),
                'cam2_to_ego': cam2_to_ego.tolist(),
                }
    ...
    def sensors(self):
        ...
        # 가로등 고정 카메라 설정 추가
        streetlight_sensors = [
                # 가로등 카메라 1: 도로를 내려다보는 첫 번째 카메라
                {
                    'type': 'sensor.camera.rgb',
                    'is_streetlight': True,  # 가로등 카메라임을 표시하는 플래그
                    'streetlight_id': 1,     # 가로등 ID
                    'x': 0.0, 'y': 0.0, 'z': 7.0,  # 고정 위치 (실제 가로등 위치로 수정 필요)
                    'roll': 0.0, 'pitch': 45.0, 'yaw': 0.0,  # 아래를 45도 각도로 내려다봄
                    'width': 1600, 'height': 900, 'fov': 100,  # 넓은 시야각
                    'id': 'STREETLIGHT_CAM_1'
                },
                # 가로등 카메라 2: 도로를 내려다보는 두 번째 카메라 (다른 각도)
                {
                    'type': 'sensor.camera.rgb',
                    'is_streetlight': True,  # 가로등 카메라임을 표시하는 플래그
                    'streetlight_id': 2,     # 가로등 ID
                    'x': 15.0, 'y': 0.0, 'z': 7.0,  # 첫 번째 카메라와 15미터 떨어진 위치
                    'roll': 0.0, 'pitch': 45.0, 'yaw': 180.0,  # 첫 번째 카메라와 반대 방향
                    'width': 1600, 'height': 900, 'fov': 100,  # 넓은 시야각
                    'id': 'STREETLIGHT_CAM_2'
                },
                # 가로등 카메라 1 뎁스
                {
                    'type': 'sensor.camera.depth',
                ...
                # 가로등 카메라 2 뎁스
                {
                    ...
                    'streetlight_id': 2,
                    'x': 15.0, 'y': 0.0, 'z': 7.0,
                    'roll': 0.0, 'pitch': 45.0, 'yaw': 180.0,
                    'width': 1600, 'height': 900, 'fov': 100,
                    'id': 'STREETLIGHT_CAM_2_DEPTH'
                },
                # 가로등 카메라 1 세그멘테이션
                {
                    'type': 'sensor.camera.semantic_segmentation',
                ...
                # 가로등 카메라 2 세그멘테이션
                {
                    ...
                    'roll': 0.0, 'pitch': 45.0, 'yaw': 180.0,
                    'width': 1600, 'height': 900, 'fov': 100,
                    'id': 'STREETLIGHT_CAM_2_SEM_SEG'
                },
        ]
        
        # 최종 사용할 센서 목록
        sensors = streetlight_sensors  # 가로등 카메라만 사용
        # sensors = original_sensors + streetlight_sensors  # 모든 센서 사용
    ...
    def save(self, near_node, far_node, near_command, far_command, tick_data, should_brake):
        ...
        # 가로등 카메라 이미지 저장
        if tick_data.get('streetlight_cam_1') is not None:
            # streetlight_cam_1 디렉토리가 없으면 생성
            (self.save_path / 'camera' / 'streetlight_1').mkdir(parents=True, exist_ok=True)
            cv2.imwrite(str(self.save_path / 'camera' / 'streetlight_1' / (f'{frame:05}.jpg')), 
                      tick_data['streetlight_cam_1'], [cv2.IMWRITE_JPEG_QUALITY, 20])
        
        if tick_data.get('streetlight_cam_2') is not None:
            # streetlight_cam_2 디렉토리가 없으면 생성
            (self.save_path / 'camera' / 'streetlight_2').mkdir(parents=True, exist_ok=True)
            cv2.imwrite(str(self.save_path / 'camera' / 'streetlight_2' / (f'{frame:05}.jpg')), 
                      tick_data['streetlight_cam_2'], [cv2.IMWRITE_JPEG_QUALITY, 20])
        
        # BEV 이미지 생성 및 저장
        bev_available = False
        if (tick_data.get('streetlight_cam_1') is not None or tick_data.get('streetlight_cam_2') is not None) and \
           (tick_data.get('streetlight_cam_1_depth') is not None or tick_data.get('streetlight_cam_2_depth') is not None):
            ...
            # numpy 배열로 변환
            cam1_to_ego = np.array(tick_data['cam1_to_ego']) if 'cam1_to_ego' in tick_data else None
            cam2_to_ego = np.array(tick_data['cam2_to_ego']) if 'cam2_to_ego' in tick_data else None
            
            # BEV 이미지 생성
            bev_image = self.generate_bev_from_streetlight(
                tick_data.get('streetlight_cam_1'),
                tick_data.get('streetlight_cam_2'),
                tick_data.get('streetlight_cam_1_depth'),
                tick_data.get('streetlight_cam_2_depth'),
                cam1_to_ego,
                cam2_to_ego
            )
            
            # BEV 이미지 저장
            cv2.imwrite(str(self.save_path / 'bev' / (f'{frame:05}.png')), bev_image)
            
            # BEV 메타데이터 준비
            bev_meta = {
                'bev_image': str(self.save_path / 'bev' / (f'{frame:05}.png')),
                'cam1_to_ego': tick_data.get('cam1_to_ego'),
                'cam2_to_ego': tick_data.get('cam2_to_ego'),
                'ego_position': [tick_data['pos'][0], tick_data['pos'][1], 0],  # x, y, z
                'ego_rotation': [0, 0, tick_data['compass']],  # roll, pitch, yaw
            ...
            with gzip.open(self.save_path / 'bev_meta' / f'{frame:05}.json.gz', 'wt', encoding='utf-8') as f:
                json.dump(bev_meta, f, indent=4)
        ...
        anno_data = {
                ...
                # BEV 관련 정보 추가
                'bev_available': 'streetlight_cam_1' in tick_data or 'streetlight_cam_2' in tick_data,
                'has_bev_image': bev_available
        }
    ...
    def get_camera_to_world_matrix(self, camera_spec):
        """
        가로등 카메라의 월드 좌표계 변환 행렬 계산
        """
        # 카메라 위치 및 회전 정보
        x, y, z = camera_spec['x'], camera_spec['y'], camera_spec['z']
        roll, pitch, yaw = camera_spec['roll'], camera_spec['pitch'], camera_spec['yaw']
        
        # Carla 회전 행렬 생성 (라디안으로 변환)
        ...
        # Carla 회전 순서: yaw (z), pitch (y), roll (x)
    ...
    def get_vehicle_to_world_matrix(self, transform):
        """
        에고 차량의 월드 좌표계 변환 행렬 계산
        """
        # 차량 위치
        x, y, z = transform.location.x, transform.location.y, transform.location.z
        
        # 차량 회전 (라디안으로 변환)
        roll_rad = np.radians(transform.rotation.roll)
        pitch_rad = np.radians(transform.rotation.pitch)
    ...
    def generate_bev_from_streetlight(self, streetlight_cam_1, streetlight_cam_2, streetlight_cam_1_depth, streetlight_cam_2_depth, cam1_to_ego, cam2_to_ego):
        """
        두 가로등 카메라로부터 BEV(Bird's Eye View) 이미지 생성
        
        Args:
            streetlight_cam_1: 첫 번째 가로등 카메라 RGB 이미지
            streetlight_cam_2: 두 번째 가로등 카메라 RGB 이미지
            streetlight_cam_1_depth: 첫 번째 가로등 카메라 깊이 이미지
            streetlight_cam_2_depth: 두 번째 가로등 카메라 깊이 이미지
            cam1_to_ego: 첫 번째 카메라에서 에고 차량으로의 변환 행렬
            cam2_to_ego: 두 번째 카메라에서 에고 차량으로의 변환 행렬
            
        Returns:
            bev_image: 생성된 BEV 이미지 (200x200 픽셀, RGB)
        """
        # BEV 이미지 크기 (UniAD와 호환되는 200x200 크기)
        bev_size = 200
        bev_range = 51.2  # meters
        resolution = 2 * bev_range / bev_size  # meters/pixel
        
        # 빈 BEV 이미지 생성 (에고 차량 중심)
        bev_image = np.zeros((bev_size, bev_size, 3), dtype=np.uint8)
        
        # 깊이 이미지가 있는 경우에만 처리
        if streetlight_cam_1_depth is not None and streetlight_cam_1 is not None:
            # 첫 번째 카메라에서 BEV로 투영
            bev_image = self._project_camera_to_bev(
                streetlight_cam_1, 
                streetlight_cam_1_depth,
                cam1_to_ego,
                bev_image,
                bev_size,
                bev_range,
        ...
        if streetlight_cam_2_depth is not None and streetlight_cam_2 is not None:
            # 두 번째 카메라에서 BEV로 투영
            bev_image = self._project_camera_to_bev(
                streetlight_cam_2, 
                streetlight_cam_2_depth,
                cam2_to_ego,
                bev_image,
                bev_size,
                bev_range,
        ...
        cv2.circle(bev_image, (center_x, center_y), 3, (0, 0, 255), -1)  # 빨간색 점
        
        return bev_image
    
    def _project_camera_to_bev(self, camera_image, depth_image, cam_to_ego, bev_image, bev_size, bev_range, resolution):
        """
        카메라 이미지를 BEV 이미지로 투영
        """
        # 카메라 내부 파라미터 (카메라 설정에서 가져와야 함)
        height, width = camera_image.shape[:2]
        f = width / 2  # 대략적인 초점 거리
        cx, cy = width / 2, height / 2  # 이미지 중심
        
        # 깊이 이미지 처리 (0-1 범위로 정규화)
        normalized_depth = np.mean(depth_image, axis=2) / 255.0  # RGB 평균
        
        # 3D 포인트 생성
        for v in range(0, height, 4):  # 계산량 감소를 위해 4픽셀마다 샘플링
            for u in range(0, width, 4):
                # 픽셀 깊이 값 (더 정확한 변환을 위해 실제 Carla 깊이로 변환해야 함)
                depth_value = normalized_depth[v, u] * 100.0  # 최대 100m 깊이 가정
                
                # 카메라 좌표계에서 3D 포인트 생성
                x_cam = (u - cx) * depth_value / f
                y_cam = (v - cy) * depth_value / f
                z_cam = depth_value
                
                # 카메라 좌표를 에고 차량 좌표로 변환
                point_cam = np.array([x_cam, y_cam, z_cam, 1.0])
                point_ego = cam_to_ego @ point_cam
                
                # 지면 위의 물체만 고려 (y 좌표가 일정 범위 내)
                if -2.0 < point_ego[1] < 2.0:
                    # 에고 차량 좌표를 BEV 픽셀 좌표로 변환
                    bev_x = int(bev_size / 2 - point_ego[0] / resolution)
                    bev_y = int(bev_size / 2 - point_ego[2] / resolution)
                    
                    # BEV 이미지 범위 내인지 확인
                    if 0 <= bev_x < bev_size and 0 <= bev_y < bev_size:
                        # BEV 이미지에 픽셀 설정 (RGB 값 사용)
                        bev_image[bev_y, bev_x] = camera_image[v, u]
        
        # 빈 공간 채우기 (선택 사항)
        bev_image = cv2.medianBlur(bev_image, 3)
