_base_ = ["/home/students/2026_CES_CTRL/UniAD/projects/configs/_base_/datasets/nus-3d.py",
          "/home/students/2026_CES_CTRL/UniAD/projects/configs/_base_/default_runtime.py"]

# 커스텀 BEV 백본으로 UniAD Stage1 설정 파일
plugin = True
...
model = dict(
    ...
    # 커스텀 BEV 백본 사용
    img_backbone=dict(
        type="UniADBEVAdapter",
        out_channels=_dim_,
        frozen_stages=1,  # 일부 레이어 고정
        norm_eval=True,
        pretrained="/home/students/2026_CES_CTRL/my_bev/carla_bev_backbone_best.pth",  # 우리가 학습시킨 모델 사용
    ),
    # FPN을 사용하지 않음 (이미 어댑터에서 다중 스케일 출력 제공)
    img_neck=None,
    freeze_img_backbone=False,  # 필요에 따라 백본 고정 여부 결정
...
data_root = "/home/students/2026_CES_CTRL/UniAD/data/nuscenes/"
info_root = "/home/students/2026_CES_CTRL/UniAD/data/infos/"
file_client_args = dict(backend="disk")
ann_file_train=info_root + "nuscenes_infos_temporal_train.pkl"
ann_file_val=info_root + "nuscenes_infos_temporal_val_3000.pkl"
ann_file_test=info_root + "nuscenes_infos_temporal_val_3000.pkl"
...
# base_track_map.py는 bevformer 체크포인트를 사용하지만, 우리는 자체 학습 모델 사용
