...
class PlanningHead(BaseModule):
    """Planning head for trajectory prediction."""
    ...
    def __init__(self,
        ...
        # Loss 설정
        self.loss_planning = loss_planning
        
        # 출력 레이어
        self.traj_embed = MLP(in_channels, in_channels, num_frames * 2, 3)
    ...
    def forward_train(self, bev_embed, img_metas, **kwargs):
        ...
        # 궤적 예측
        trajectory = self.traj_embed(hs)
        trajectory = trajectory.view(-1, self.num_frames, 2)
        
        # 손실 계산
        losses = self.loss(trajectory, img_metas, **kwargs)
    ...
    def forward_test(self, bev_embed, img_metas, **kwargs):
        ...
        # 결과 생성
        results = self.get_trajectory(trajectory, img_metas, **kwargs)
        
        return results
        
    def loss(self, trajectory, img_metas, **kwargs):
        """Compute losses."""
        losses = dict()
        
        # 궤적 예측 손실
        loss_planning = self.loss_planning(trajectory, **kwargs)
        losses['loss_planning'] = loss_planning
        
        return losses
        
    def get_trajectory(self, trajectory, img_metas, **kwargs):
