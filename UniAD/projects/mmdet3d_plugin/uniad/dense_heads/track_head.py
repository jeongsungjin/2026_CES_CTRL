...
class TrackHead(BaseModule):
    """Track head for object tracking."""
    ...
    def __init__(self,
        ...
        # 출력 레이어
        self.cls_embed = MLP(in_channels, in_channels, num_classes, 3)
        self.reg_embed = MLP(in_channels, in_channels, 4, 3)
    ...
    def forward_train(self, bev_embed, img_metas, **kwargs):
        ...
        # 분류 및 회귀 예측
        cls_scores = self.cls_embed(hs)
        bbox_preds = self.reg_embed(hs)
        
        # 손실 계산
        losses = self.loss(cls_scores, bbox_preds, img_metas, **kwargs)
    ...
    def forward_test(self, bev_embed, img_metas, **kwargs):
        ...
        # 결과 생성
        results = self.get_bboxes(cls_scores, bbox_preds, img_metas, **kwargs)
        
        return results
        
    def loss(self, cls_scores, bbox_preds, img_metas, **kwargs):
        """Compute losses."""
        losses = dict()
        
        # 분류 손실
        loss_cls = self.loss_track['loss_cls'](cls_scores, **kwargs)
        losses['loss_cls'] = loss_cls
        
        # 회귀 손실
        loss_bbox = self.loss_track['loss_bbox'](bbox_preds, **kwargs)
