...
class ConvLSTM(nn.Module):
    def __init__(self, input_dim=3, hidden_dim=64, kernel_size=3, pred_frames=5):
        super().__init__()
...
def split_channels(frames):
    ...
    return np.stack([bg, road, vehicle], axis=1)  # (T, 3, H, W)
...
class OccupancyGridDataset(Dataset):
    def __init__(self, npy_file, input_frames=10, pred_frames=5, step=1, augment=False):
        self.raw_data = split_channels(np.load(npy_file))
...
def main():
    ...
    model = ConvLSTM(input_dim=3, pred_frames=pred_frames).to(device)
